...
calculate_changes <- function(input, save_results, section) {
  # Auxiliary functions ------------------------------------------------------------------
  # read_data()
  ...
  coe_extraction <- function(x) {
    result <- lapply(x, function(x) data.frame(t(x[["coefficients"]]), summary(x)[["sigma"]]))
  }
  ...
  coe_merge <- function(x) {
    ...
    colnames(result) <- c(
      "Regresion", "Intercept", "Temp",
      "Sal", "AOU", "RMSE", "Ni", "TA", "Si"
    )
    result <- subset(result, select = c(1:5, 7:9, 6))
    result
  }
  ...
  model_DICabio <- function(x) {
    variables <- c("ptemp", "Sal", "AOU", "Ni", "TA", "Si")
    # 3 variables
    tres <- combn(variables, 3)
    list3 <- vector(mode = "list", length = ncol(tres))
    for (n in 1:(ncol(tres))) {
      list3[[n]] <- formula(
        paste0(
          "DICabio",
          " ~ ",
          paste(tres[1, n], tres[2, n], tres[3, n], sep = " + ")
        )
      )
    }
    # 4 variables
    cuatro <- combn(variables, 4)
    list4 <- vector(mode = "list", length = ncol(cuatro))
    for (n in 1:(ncol(cuatro))) {
      list4[[n]] <- formula(
        paste0(
          "DICabio",
          " ~ ",
          paste(cuatro[1, n], cuatro[2, n], cuatro[3, n], cuatro[4, n], sep = " + ")
        )
      )
    }
    # 5 variables
    cinco <- combn(variables, 5)
    list5 <- vector(mode = "list", length = ncol(cinco))
    for (n in 1:(ncol(cinco))) {
      list5[[n]] <- formula(
        paste0(
          "DICabio",
          " ~ ",
          paste(cinco[1, n], cinco[2, n], cinco[3, n], cinco[4, n], cinco[5, n], sep = " + ")
        )
      )
    }
    # 6 variables
    list6 <- formula("DICabio ~ ptemp + Sal + AOU + Ni + TA + Si")
    # total
    lista <- c(list3, list4, list5, list6)
    regression <- vector(mode = "list", length = length(lista))
    for (n in 1:(length(lista))) {
      regression[[n]] <- rlm(lista[[n]], data = x, psi = psi.bisquare, maxit = 200)
    }
    regression
  }
  ...
  emlr_DICabio <- function(x) {
    ...
    for (n in 1:ncol(combinaciones)) {
      ...
      coeficientes_finales[[n]] <- data.frame(
        tiempo2[, 1:2],
        tiempo2[, 3:9] - tiempo1[, 3:9],
        RMSE2 = tiempo2[, 10],
        RMSE1 = tiempo1[, 10],
        tRMSE = tiempo2[, 10] + tiempo1[, 10]
      )
    }
    coeficientes_finales
  }
  ...
  Cant <- function(x, y) {
    ...
    for (i in 1:ncol(combinaciones)) {
      ...
      for (j in 1:length(df)) {
        coe <- as.data.frame(subset(y[[i]], Isopycnal == names(df)[[j]]))
        coe[is.na(coe)] <- 0
        Temp <- df[[j]][, 13]
        Sal <- df[[j]][, 6]
        Si <- df[[j]][, 9]
  ...
  consolidate <- function(x, y) {
    ...
    for (i in 1:ncol(combinaciones)) {
      ...
      result[[i]] <- data.frame(
        original_data,
        media = apply(antropogenic_carbon, 1, mean),
        sd = apply(antropogenic_carbon, 1, sd)
      )
    }
    result
  }
  ...
  coefficients_Cant <- lapply(data, lapply, model_DICabio) %>%
    ...
    lapply(., bind_rows, .id = "Isopycnal") %>%
    emlr_DICabio(.) %>%
    lapply(., group_by, Isopycnal) %>%
    lapply(., top_n, -10, wt = tRMSE)
  names(coefficients_Cant) <- coefficient_names

  carbono_Cant <- lapply(1:10, Cant, coefficients_Cant) %>%
  ...
  for (n in 1:length(carbono_Cant)) {
    carbono_antro <- data.frame(carbono_Cant[[n]][, 23:24])
    names(carbono_antro) <- c("Ca", "Ca_sd")
    C_AOU <- carbono_AOU[[n]]
    names(C_AOU) <- "C_AOU"
